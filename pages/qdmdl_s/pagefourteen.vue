        ...
        <div style="margin-left:66px">
          ...
          <div>
            <div  style="display: flex; align-items: center;justify-content: center">
              <span @click="predict2()" class="draw_button" style="margin:20px 0 0px 0px">周边孔超挖量和爆破块度预测</span>
            </div>
            <h3 class="font-type8" ><strong>线性超挖预测值为：</strong>
              <br>
...
<!-- 第二部分开始 -->
      <div style="margin: 20px 0 0 60px;width:100%;height:auto;">
        <div>
          <h3 class="font-type8" style="margin-left: 7px"><strong>平均块度预测值为：</strong>
            <br>
            <el-input style="width:1167px;margin-top: 10px" v-model="avgFragment"><template slot="suffix">m</template></el-input>
          </h3>
          <h3 class="font-type8" style="margin-left: 7px"><strong>最大块度预测值为：</strong>
            <br>
            <el-input style="width:1167px;margin-top: 10px" v-model="maxFragment"><template slot="suffix">m</template></el-input>
          </h3>
        </div>
...
<!--        表格开始-->
      <div style="width:100%;margin-left: 67px;margin-top: 20px">
        <el-table :data="tableData"
                  style="width: 1165px;border: 1px solid rgba(35,73,109,1)"
      ...
      <div style="margin-left: 70px;margin-top: -400px">
        <el-table
            :data="tablesData1"
            size="large"
            style="width: 1160px;border: 1px solid rgba(35,73,109,1);margin-top: 440px;font-size: 20px"
            :cell-style="cellStyle1"
            :header-cell-style="{ color: 'white', borderColor: '#5B9BD5',backgroundColor:'#01366a'}">

          <el-table-column prop="ThreeD" label="爆破块度分布曲线" align="center" width="1159px">
      ...
      <div style="display: flex; align-items: center;justify-content: center;margin-left: 63px;margin-top: 5px">
        <span @click="predict3()" class="draw_button" style="margin:20px 0 0px 0px">掏槽爆破槽腔范围预测</span>
      </div>
      <div style="margin-left: 70px;margin-top: -415px">
        <el-table
            :data="tablesData1"
            size="large"
            style="width: 1160px;border: 1px solid rgba(35,73,109,1);margin-top: 440px;font-size: 20px"
          ...
          <el-table-column prop="ThreeD" label="掏槽爆破槽腔范围预测" align="center" width="1159px">
            <template slot-scope="scope" >
              <!-- 导入 图片  -->
              <img v-show="imageSrc"  id="backendImage" :src="imageSrc" style="width: 700px;height: 500px;" alt="HeatMap image" >
            </template>
          </el-table-column>
        </el-table>
      </div>
...
export default {
    components:{show, tables,lumpinessview},
    data(){
        return{
          I:'',
          tablesData1:[
  ...
  created() {
    ...
    methods:{
      cellStyle({ row, column, rowIndex, columnIndex }) {
        return "height:20px!important; border:1px solid rgba(35,73,109,1)!important;border-bottom:none; color:#FFFFFF!important; padding-left:0px!important;background-color:#011635;";
      },
      ...
      predict2(){
        ...
        predictApi.calOverExcavation(this.projectId).then(response => {
          ...
          this.I = parseFloat(response.data.data).toFixed(2);
        })
