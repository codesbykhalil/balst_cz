<template>
  <div class="card5_1">
    <div>
      <h2 class= title-before style="margin-top: 21px;padding-top:21px;font-size: 20px;!important;color: white;margin-left: 40px;">7 辅助孔布置</h2>
      <div style="margin-top:21px">
        <h3 class="font-type11" style="display:inline-block;margin-top: 21px;color: white">装药直径：
          <el-input style="width:1160px;padding-top: 10px" v-model="auxDc"><template slot="suffix">mm</template></el-input>
        </h3>
        <br/>
        <h3 class=" font-type11" style="display:inline-block;margin-top: 21px;color: white">排距系数：
          <el-input style="width:1160px;padding-top: 10px" v-model="ratioRow"></el-input>
        </h3>
        <br/>
        <h3 class="font-type11" style="display:inline-block;margin-top: 21px;color: white">孔距系数：
      ...
      <div class="proovr1-collect__tags" style="display: flex; align-items: center;">
        ...
        <span @click="calculate" class="proovr1-collect__tag" style="margin:30px 0 20px 595px">计算与绘图</span>
...
export default {
  ...
  methods:{
    ...
    calculate(){
      this.timer = new Date().getTime()
      this.projectId = cookie.get("upProjectId");
      console.log("11页面 upProjectId"+this.projectId);
