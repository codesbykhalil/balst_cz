...
<el-container class="operation-wrapper_1" >
  ...
  <div class="top-box">
      ...
      <el-menu-item index="1" style="font-size: 30px;color: #FFFFFF;">上台阶楔形掏槽爆破方案设计</el-menu-item>
...
export default {
  ...
  created() {
    // 从 cookie 中获取之前存储的数据
    const upData = cookie.get('upData');
    if (upData) {
      // 将数据设置到组件的数据中，以便在页面上显示
      this.upDatas = JSON.parse(upData);
    }
    //云端文件导入回显
