<template>
  <div class="card8_1">
    <div>
      <h2 class= title-before style="margin-top: 21px;padding-top:21px;font-size: 20px;!important;color: black;margin-left: 40px;">12 装药量计算</h2>

      <div class="proovr-collect__tags" style="display: flex; align-items: center;">
        <span class="proovr-collect__tag" style="margin:35px 0 0 60px;width:200px" @click="calculate()">炮孔统计制表</span>
      </div>
      ...
      <div>
        <h3  style=" margin-left: 63px;
                ...
                width: 400px;margin-top: 15px">总药量:
          <el-input style="width:1160px" v-model="AllExplosiveAmount" :readonly="true"></el-input>
        </h3>
        <h3   style=" margin-left: 63px;
                font-size: 18px !important;
                width: 400px;margin-top: 12px">&ensp;&ensp;单耗:
          <el-input style="width:1160px;margin-top: 12px" v-model="unitCon" :readonly="true"></el-input>
        </h3>
      </div>
    </div>
  </div>
    ...
    methods:{
        calculate(){
          ...
          calculateApi.getUnitCon(this.projectId).then(response => {
            ...
            this.unitCon = response.data.data
          })
        },
