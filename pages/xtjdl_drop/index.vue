...
<el-container class="operation-wrapper_1" >
  ...
  <div class="top-box">
    ...
    <div class="header_title"><span style="color: #FFFFFF;font-size: 35px">下台阶+楔形掏槽</span></div>
...
export default {
  ...
  methods:{
    ...
    fetchProjects() {
      ...
      getProject.get_relate_project(this.save_relate_project).then(res=>{
        console.log(res.data.data.stepDownList);
        this.projects=[];
        this.newProjects=[];
        console.log('API Response:', res);
        res = res.data.data.stepDownList;
  ...
  mounted() {
    ...
    if (!cookie.get('dwProjectId'))//判断是否是第一次加载下台阶页面
    {
     //id也清空，生成一个dwProjectId
      getProject.getProject(projectId).then(response => {
        ...
        console.log(JSON.stringify(this.saveProjectMessage));
        //判断是2或者3台阶
        if (this.saveProjectMessage.stepLine2==null||this.saveProjectMessage.stepLine2=='')
        {
          console.log("此时为2台阶: ");
          this.saveProjectMessage.stepLine1 = response.data.data.stepLine1;
          console.log("下台阶高度：" + this.saveProjectMessage.stepLine1);
        } else {
          console.log("此时为3台阶: ");
          this.saveProjectMessage.stepLine1='';
          console.log("下台阶高度："+this.saveProjectMessage.stepLine2);
        }
        calculateApi.saveProject(this.saveProjectMessage).then(response => {
          if (response.data.code == 1) {
            console.log(response.data.data);
            cookie.set("dwProjectId", response.data.data.id);
            this.projectId = cookie.get("dwProjectId");//获取下台阶projectId
            this.save_relate_project.stepDownId = this.projectId;
            getProject.save_relate_project(this.save_relate_project).then(res=>{
              console.log(res.data.data);
            })
            this.dwProjectId = this.projectId;
            this.fetchProjects();
            // 调用后端 API 获取轮廓线信息
      ...
      getProject.getProject(projectId).then(response=>{
        if (response.data.code == 1){
          ...
          this.save_relate_project.stepDownId =cookie.get('dwProjectId');
          getProject.save_relate_project(this.save_relate_project).then(res=>{
            console.log(res.data.data);
          })
          this.saveProjectMessage.excavationCode = "001005";
    ...
    dwProjectId:{
      handler(newVal){
        ...
        if (newVal !=''&& newVal!=null){
          console.log("dwProjectId:"+newVal);
          //调用fetchProjects()方法加载历史记录
          this.fetchProjects();
        }
      },
      immediate: true
...
.project-history {
  padding: 10px;
  background-color: #111537;
  height: 720px; /* 设置固定高度以启用滚动 */
  overflow-y: auto;
  color: white; /* 字体颜色与界面一致 */
}
/* 隐藏滚动条，但仍然可以滚动 */
.project-history {
